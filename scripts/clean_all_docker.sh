...
# clean up flows
flow_ids=(`docker compose run -it --rm web globus flows run list | grep 'AERO' | awk '{print $1;}'`)
for fidx in "${flow_ids[@]}"; do docker compose run -it --rm web globus flows run delete $fidx; done
